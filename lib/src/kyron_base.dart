...
import 'package:kyron/src/notification.dart';
...
/// notifications within an application. It promotes loose coupling by decoupling
/// the senders of requests/notifications from their handlers.
///
...
///     that handle requests and notifications, as well as the pipeline behaviors
...
///     the handler.
/// 4.  Notification Publishing: Handles outgoing notifications ([Notification]) via
///     the [publish] method. It identifies all registered handlers for the specific
///     notification type and invokes them in a defined order.
///
...
final class Kyron implements KyronInterface {
  ...
  /// class InfoNotification extends Notification { final String info; InfoNotification(this.info); }
  ...
  /// Registers a factory function responsible for creating or providing a handler
  /// for a specific notification type ([TNotification]).
  ///
  /// When [publish] is called with a notification object of type [TNotification],
  ...
  /// The use of a factory function ([handlerFactory]) instead of directly accepting
  /// a handler instance offers crucial advantages:
  /// *   **Lifecycle Management & Isolation:** Allows creating new handler instances per notification if needed.
  ...
  ///
  /// **Multiple Handlers:** Unlike requests ([send]/[stream]), a single notification
  /// type can have **multiple** registered handlers. All applicable handlers will
  /// be executed when the notification is published.
  ///
  ...
  ///
  /// **Generics:**
  ///   - [TNotification]: The specific type of the notification message, which must
  ///     extend [Notification].
  ///
  /// **Parameters:**
  ///   - [handlerFactory]: A zero-argument function (`()`) that returns an instance
  ...
  ///     other handlers for the same notification type. Defaults to
  ...
  ///
  /// // Usage
  /// print('\nPublishing OrderPlaced notification...');
  ...
  @override
  Future<void> publish(Notification notification) async {
    final notificationType = notification.runtimeType;
    final correlationId = notification.hashCode;
    _log.info(
      'Publishing notification [$correlationId] of type $notificationType.',
    );
